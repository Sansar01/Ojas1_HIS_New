...
const VARIANTS: Record<Variant, string> = {
  ...
  secondary: "bg-ink-900 text-white hover:bg-ink-800 shadow-[0_10px_20px_-14px_rgba(10,23,28,.9)]",
  outline: "border border-ink-200 bg-white text-ink-700 hover:border-brand-300 hover:bg-brand-25 hover:text-brand-700",
  ghost: "text-ink-600 hover:bg-ink-50 hover:text-ink-900",
  danger: "bg-coral-500 text-white hover:bg-coral-600 shadow-[0_10px_20px_-14px_rgba(217,74,74,.9)]",
  success: "bg-mint-500 text-white hover:bg-mint-600 shadow-[0_10px_20px_-14px_rgba(31,157,99,.9)]",
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  { className, variant = "primary", size = "md", loading, icon, iconRight, block, children, disabled, ...rest },
  ref,
) {
  return (
    <button
      ...
      {...rest}
    >
      {loading ? <Loader2 className="size-4 animate-spin" aria-hidden /> : icon ? <span className="[&>svg]:size-4">{icon}</span> : null}
      {children}
...
export type Tone = "brand" | "neutral" | "mint" | "amber" | "coral" | "lagoon" | "ink";
...
export function Avatar({
  ...
  const dims = { xs: "size-6 text-[10px]", sm: "size-8 text-[11px]", md: "size-10 text-[13px]", lg: "size-14 text-base", xl: "size-20 text-2xl" }[size];
...
export function Panel({
  ...
  return <As className={cn("rounded-xl border border-ink-100 bg-white shadow-card", className)}>{children}</As>;
}
  ...
  return (
    <div className={cn("flex flex-wrap items-start justify-between gap-3 border-b border-ink-100 px-5 py-4", className)}>
      <div className="flex min-w-0 items-start gap-3">
        {icon && <span className="mt-0.5 grid size-9 place-items-center rounded-lg bg-brand-50 text-brand-600 [&>svg]:size-4.5">{icon}</span>}
        <div className="min-w-0">
          <h3 className="truncate text-[15px] font-semibold text-ink-900">{title}</h3>
          {subtitle && <p className="mt-0.5 text-[12.5px] leading-relaxed text-ink-400">{subtitle}</p>}
        </div>
      </div>
...
export function Progress({ value, tone = "brand", className }: { value: number; tone?: "brand" | "amber" | "coral" | "mint"; className?: string }) {
  const bg = { brand: "bg-brand-500", amber: "bg-amberly-500", coral: "bg-coral-500", mint: "bg-mint-500" }[tone];
  return (
    <div className={cn("h-1.5 w-full overflow-hidden rounded-full bg-ink-100", className)}>
      <div className={cn("h-full rounded-full transition-[width] duration-700 ease-out", bg)} style={{ width: `${Math.min(100, Math.max(0, value))}%` }} />
    </div>
  );
}

export function Divider({ className, vertical }: { className?: string; vertical?: boolean }) {
  return <div className={cn(vertical ? "w-px self-stretch" : "h-px w-full", "bg-ink-100", className)} />;
}

export function Kpi({ label, value, hint, tone = "ink" }: { label: string; value: React.ReactNode; hint?: React.ReactNode; tone?: Tone }) {
  return (
    <div className="flex flex-col gap-1">
      <span className="text-[11px] font-medium uppercase tracking-[0.14em] text-ink-400">{label}</span>
      <span className={cn("num text-xl font-semibold", tone === "coral" ? "text-coral-600" : tone === "mint" ? "text-mint-600" : "text-ink-900")}>{value}</span>
